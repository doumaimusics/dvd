...
$(function() {
	...
	$("#wechat-fw-01 area").click(function() {
		...
		//$(".Capacity").eq(trIndex*5+index).show(); 要是不用数组写  从html页面获取就这样写
		
		my.confirms(trIndex*5+index);
		$(".Capacity").show();
		$("body .mask").show();
		$("body .close").show();
	})
...
"17.Final inspection",
		...
		var wordCons = [
			...
			"Stage 3 is to transfer the image using an artwork film to the board surface, using photosensitive dry-film and UV light, which will polymerise the dry film exposed by the artwork.This step of the process is performed in a clean room.",
			"Stage 2 is to remove the unwanted copper from the panel using etching. Once this copper has been removed, the remaining dry film is then removed leaving behind the copper circuitry that matches the design.<br>蚀刻是用化学或电化学方法去除基材上无用导电材料，形成印制图形的工艺", "2.制备照相底版是PCB生产中的关键步骤，其质量直接影响到最终产品的质量；现在一般都用计算机辅助设计直接在光绘仪上绘制照相底版。一般要有三种照相底图：<br>（1）导电图形底图<br>（2）阻焊图形底图<br>（3）标记字符底图",
...
//	var confirmsHtmls = '<img src=' + icon[index].src + '/ class="icon"><h3 class="title">' + titles[index] + '</h3><div class="centerbox"><p class="present">' + wordCons[index] + '</p><ul><li><img src="images/capabilities/' + imgCons[index].one + '" /></li><li><img src="images/capabilities/' + imgCons[index].two + '" /></li></ul></div>';
